...
def renderizar_dashboard_gestao(setor_atual):
    ...
    for f in arquivos:
        with open(f, "r", encoding="utf-8") as file:
            ...
            if conteudo.get("area") == setor_atual:
                nome_colab = os.path.basename(f).replace(".json", "").replace("_", " ").title()
                
                row = {"Colaborador": nome_colab}
                for idx, nota in enumerate(conteudo.get("notas_c", [])):
                    pilar = perguntas_data[idx]["pilar"]
                    row[f"P_{idx+1}_{pilar}"] = nota
                
                row["Média Geral"] = sum(conteudo.get("notas_c", [])) / len(perguntas_data)
